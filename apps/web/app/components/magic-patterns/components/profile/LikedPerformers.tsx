...
export const LikedPerformers = ({
  ...
  return <div>
      ...
      {likedPerformers.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {likedPerformers.map((performer: any) => <div key={performer.id} className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate(`/performers/${performer.id}`)}>
          ...
          <button onClick={() => navigate('/performers')} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-purple-600 hover:bg-purple-700">
            Discover Performers
          </button>
        </div>}
    </div>;
