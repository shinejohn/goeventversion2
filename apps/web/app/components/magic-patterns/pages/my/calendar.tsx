...
import { CalendarIcon, ChevronLeftIcon, ChevronRightIcon, ListIcon, ClockIcon, MapPinIcon, UsersIcon, TagIcon, PlusIcon, FilterIcon, DownloadIcon, ShareIcon } from 'lucide-react';
...
const MyCalendar = () => {
  ...
  // Function to get the selected event
  const getSelectedEvent = () => {
    return mockEvents.find(event => event.id === showEventDetails);
  };
  return <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <div className="px-4 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center">
              <div className="mr-4 flex">
                ...
                <button onClick={() => setViewMode('week')} className={`px-3 py-1 text-sm ${viewMode === 'week' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Week
                </button>
                <button onClick={() => setViewMode('day')} className={`px-3 py-1 text-sm ${viewMode === 'day' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Day
                </button>
              ...
              <div className="flex space-x-2">
                <button className="p-1 rounded-full hover:bg-gray-100" title="Filter Events">
                  <FilterIcon className="h-5 w-5 text-gray-600" />
                </button>
                <button className="p-1 rounded-full hover:bg-gray-100" title="Add Event">
                  <PlusIcon className="h-5 w-5 text-gray-600" />
                </button>
                <button className="p-1 rounded-full hover:bg-gray-100" title="Export Calendar">
                  <DownloadIcon className="h-5 w-5 text-gray-600" />
                </button>
                <button className="p-1 rounded-full hover:bg-gray-100" title="Share Calendar">
          ...
          {/* Calendar Body */}
          {viewMode === 'month' && <div className="bg-white">
              {/* Day Names */}
              ...
              {/* Calendar Grid */}
              <div className="grid grid-cols-7 grid-rows-6 h-[36rem]">
              ...
              return <div key={index} className={`border-b border-r border-gray-200 p-1 ${day.isCurrentMonth ? 'bg-white' : 'bg-gray-50'} ${day.isToday ? 'bg-blue-50' : ''}`}>
                      ...
                      <div className="mt-1 space-y-1 max-h-24 overflow-y-auto">
                        {events.map(event => <button key={event.id} className={`w-full text-left px-2 py-1 rounded text-xs ${event.color} ${event.textColor} truncate border-l-2`} onClick={() => handleEventClick(event.id)}>
          ...
          {viewMode === 'list' && <div className="divide-y divide-gray-200">
              {mockEvents.length === 0 ? <div className="py-12 text-center">
                  ...
                  <div className="mt-6">
                ...
                </div> : mockEvents.map(event => <div key={event.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 cursor-pointer" onClick={() => handleEventClick(event.id)}>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center">
                        {event.attending > 0 && <div className="flex items-center mr-4 text-sm text-gray-500">
                            <UsersIcon className="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400" />
                            <span>
                              {event.attending}{' '}
                              {event.attending === 1 ? 'friend' : 'friends'}
                            </span>
                          </div>}
      ...
      {/* Event Details Modal */}
      {showEventDetails && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            ...
            <div className="space-y-4">
              <div className="flex items-center">
                ...
                <span className="text-gray-700">
                  {getSelectedEvent()?.date.toLocaleDateString('en-US', {
                weekday: 'long',
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              ...
              <div className="flex items-center">
                ...
                <span className="text-gray-700">
                  {getSelectedEvent()?.startTime} -{' '}
                  {getSelectedEvent()?.endTime}
                </span>
              </div>
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <div className="text-gray-500 text-sm">
                    {getSelectedEvent()?.address}
                  </div>
                </div>
              </div>
              <div className="flex items-center">
                <TagIcon className="h-5 w-5 text-gray-400 mr-2" />
                <span className={`px-2 py-1 text-xs rounded-full ${getSelectedEvent()?.color} ${getSelectedEvent()?.textColor}`}>
                  {getSelectedEvent()?.category}
                </span>
              </div>
              {getSelectedEvent()?.attending > 0 && <div className="flex items-center">
                  <UsersIcon className="h-5 w-5 text-gray-400 mr-2" />
                  <span className="text-gray-700">
                    {getSelectedEvent()?.attending} friends attending
                  </span>
                </div>}
            </div>
            <div className="mt-6 flex justify-end space-x-3">
              <button className="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50" onClick={() => navigateTo(`/event?id=${getSelectedEvent()?.id}`)}>
                View Event Details
              </button>
              <button className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                {getSelectedEvent()?.reminder ? 'Edit Reminder' : 'Set Reminder'}
              </button>
            </div>
          </div>
        </div>}
