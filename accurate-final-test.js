...
class AccurateTester {
  ...
  async testPage(url, expectedAuth = false) {
    ...
    try {
      ...
      if (result.title && result.title !== 'undefined') {
        ...
        // Check for actual data/content on the page
        const pageContent = await this.page.content();
        const hasData = pageContent.includes('Community') || 
                       pageContent.includes('Event') || 
                       pageContent.includes('Performer') || 
                       pageContent.includes('Venue') ||
                       pageContent.includes('Calendar') ||
                       pageContent.includes('Shop') ||
                       pageContent.includes('Sign In') ||
                       pageContent.includes('Sign Up') ||
                       pageContent.includes('Hub') ||
                       pageContent.includes('Discover') ||
                       pageContent.includes('Total Communities') ||
  ...
  async generateReport() {
    ...
    // Data Analysis
    console.log('\n📊 DATA ANALYSIS:');
    const pagesWithData = this.results.filter(p => p.status === 'working' && p.hasData).length;
    const pagesWithoutData = this.results.filter(p => p.status === 'working' && !p.hasData).length;
    ...
    this.results
      ...
      .forEach(r => console.log(`      - ${r.name} (${r.url}) - Data: ${r.hasData ? 'Yes' : 'No'}`));
    ...
    if (pagesWithoutData > 0) {
      console.log('   5. Fix data loading issues on working pages');
    }

    if (errors === 0 && authIssues === 0 && pagesWithoutData === 0) {
